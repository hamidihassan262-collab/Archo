...
void main() {
  ...
  float mouseWarp = 0.0;
  if (uEnableMouse) {
    mouseWarp = uMouseInfluence * 0.15 * exp(-mDist * mDist * 2.0);
  }

  float warpAx = coords.x + displaceA(coords.y, halfT) * uWarpIntensity + (coords.x - mPos.x) * mouseWarp;
  float warpAy = coords.y - displaceA(coords.x * cos(fullT) * 1.235, halfT) * uWarpIntensity + (coords.y - mPos.y) * mouseWarp;
  float warpBx = coords.x + displaceB(coords.y, halfT) * uWarpIntensity + (coords.x - mPos.x) * mouseWarp;
  float warpBy = coords.y - displaceB(coords.x * sin(fullT) * 1.235, halfT) * uWarpIntensity + (coords.y - mPos.y) * mouseWarp;
